...
The default seeded admin **password is printed to console on first run** — copy it before scrollback eats it.

## Solution layout (clean architecture, 4 projects + 2 test projects)

| Project | Role |
|---|---|
| `Workshop.Domain` | Entities, enums, value objects, `InsuranceCaseStateMachine` (Stateless 5.x). No EF, no MediatR. |
| `Workshop.Application` | MediatR commands/queries/handlers, FluentValidation validators, DTOs, abstractions (`IWorkshopDbContext`, `IEmailSender`, `ISmsSender`, `IMyDataClient`, `IFileStore`). |
...
**MediatR pipeline order:** `SerializeRequestsBehavior` → `ValidationBehavior` → handler. The serialize behavior holds a per-DI-scope `SemaphoreSlim` because `WorkshopDbContext` is circuit-scoped in Blazor Server and components fire `Mediator.Send` concurrently during a single render pass — without serialization EF throws "A second operation was started on this context instance". Handlers stay lock-free.
...
- **No REST controllers.** Components call MediatR handlers directly. If you find yourself reaching for an HTTP client, you're probably already inside the app.
- **Commands/queries are MediatR records** in `src/Workshop.Application/Features/<Entity>/`. Convention: `Create<Entity>Command`, `Update<Entity>Command`, `Delete<Entity>Command`, `Get<Entity>ByIdQuery`, `List<Entity>Query`. Each gets a handler and (for commands) a FluentValidation `AbstractValidator` in the same file.
...
- Account management UI is bare — only login/logout. No password reset / MFA / staff admin / role assignment UI.
